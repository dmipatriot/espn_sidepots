...
def test_update_efficiency_streaming_totals():
    ...
    update_efficiency(stats, week_one)
    update_efficiency(stats, week_two)
    ...
    report = format_efficiency_report(labels, stats)
    lines = report.splitlines()
    assert lines[0] == "Season Efficiency"
    assert len(lines) == 4
    assert all("Actual" in line and "Optimal" in line and "Eff" in line for line in lines[1:])
